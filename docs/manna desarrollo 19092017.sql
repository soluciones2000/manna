...
--
-- Servidor: localhost
-- Tiempo de generación: 19-09-2017 a las 15:30:25
...
--
-- Estructura Stand-in para la vista `v_org`
-- (Véase abajo para la vista actual)
--
CREATE TABLE `v_org` (
`id` bigint(20)
...
,`afiliado` varchar(20)
,`lado` varchar(20)
,`tit_nombres` varchar(150)
,`tit_apellidos` varchar(150)
);

-- --------------------------------------------------------

--
-- Estructura Stand-in para la vista `v_org2`
-- (Véase abajo para la vista actual)
--
CREATE TABLE `v_org2` (
`id` bigint(20)
...
,`tit_nombres` varchar(150)
,`tit_apellidos` varchar(150)
,`afil_nombres` varchar(150)
,`afil_apellidos` varchar(150)
);

-- --------------------------------------------------------

--
-- Estructura Stand-in para la vista `v_org3`
-- (Véase abajo para la vista actual)
--
CREATE TABLE `v_org3` (
`id` bigint(20)
...
--
-- Estructura para la vista `v_org`
--
DROP TABLE IF EXISTS `v_org`;

CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `v_org`  AS  select `organizacion`.`id` AS `id`,`organizacion`.`organizacion` AS `organizacion`,`organizacion`.`nivel` AS `nivel`,`organizacion`.`afiliado` AS `afiliado`,`organizacion`.`lado` AS `lado`,`afiliados`.`tit_nombres` AS `tit_nombres`,`afiliados`.`tit_apellidos` AS `tit_apellidos` from (`organizacion` left join `afiliados` on((`organizacion`.`organizacion` = `afiliados`.`tit_codigo`))) ;

-- --------------------------------------------------------

--
-- Estructura para la vista `v_org2`
--
DROP TABLE IF EXISTS `v_org2`;

CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `v_org2`  AS  select `v_org`.`id` AS `id`,`v_org`.`organizacion` AS `organizacion`,`v_org`.`nivel` AS `nivel`,`v_org`.`afiliado` AS `afiliado`,`v_org`.`lado` AS `lado`,`v_org`.`tit_nombres` AS `tit_nombres`,`v_org`.`tit_apellidos` AS `tit_apellidos`,`afiliados`.`tit_nombres` AS `afil_nombres`,`afiliados`.`tit_apellidos` AS `afil_apellidos` from (`v_org` left join `afiliados` on((`v_org`.`afiliado` = `afiliados`.`tit_codigo`))) ;

-- --------------------------------------------------------

--
-- Estructura para la vista `v_org3`
--
DROP TABLE IF EXISTS `v_org3`;

CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `v_org3`  AS  select `v_org`.`id` AS `id`,`v_org`.`organizacion` AS `organizacion`,`v_org`.`nivel` AS `nivel`,`v_org`.`afiliado` AS `afiliado`,`v_org`.`lado` AS `lado`,`v_org`.`tit_nombres` AS `tit_nombres`,`v_org`.`tit_apellidos` AS `tit_apellidos`,`afiliados`.`tit_nombres` AS `afil_nombres`,`afiliados`.`tit_apellidos` AS `afil_apellidos`,`afiliados`.`tipo_afiliado` AS `tipo_afiliado` from (`v_org` left join `afiliados` on((`v_org`.`afiliado` = `afiliados`.`tit_codigo`))) ;
