...
echo '<div id="cuerpo">';
	echo '<div style="text-align:center">';
		...
		echo '<font color="red">'.trim($codigo).' - '.trim($name_user).'</font></h3>';
	echo '</div>';
...
while($row = mysql_fetch_array($result)) {
	...
	if ($patroc_codigo<>$row['patroc_codigo']) {
		$txt .= '<div style="text-align:right;padding-right:5.5%;">'.str_repeat('-', 20)."</div>";
		$txt .= '<div style="text-align:right;padding-right:5.5%;"><i>Total Patrocinado '.$tit_codigo." - ".trim($tit_nombre_completo).': '.trim(number_format($tot_tit,2,',','.'))."</i></div>";
		$txt .= '<div style="text-align:right;padding-right:5.5%;">'.str_repeat('=', 20)."</div>";
		$txt .= '<div style="text-align:right;padding-right:5.5%;"><b>Total Patrocinador '.$patroc_codigo." - ".trim($patroc_nombres).': '.trim(number_format($tot_pat,2,',','.'))."</b></div>";
		echo $txt;
		$patroc_codigo = $row['patroc_codigo'];
		$tit_codigo = $row['tit_codigo'];
		$patroc_nombres = $row['patroc_nombres'];
