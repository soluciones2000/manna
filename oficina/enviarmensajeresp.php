...
*/
$emails = (isset($_POST["emails"])) ? $_POST["emails"] : "sgcvzla@gmail.com,soluciones2000@gmail.com,luisrodriguezestrada@hotmail.com" ;
$pos = 0;
$arb = 0;
$pto = 0;
$dirx = "";
...
if (strlen($emails)>0) {
   ...
   for ($i=0; $i < $largo; $i++) {
      $pos = strpos($emails,',',$i);
      $len = $pos-$i;
      echo 'pos '.$pos.'<br>';//sgcvzla@gmail.com,soluciones2000@gmail.com,luisrodriguezestrada@hotmail.com
//      if ($pos>0 and $pos<=strlen($emails)) {
      if ($pos>0) {
         $dirx = substr($emails,$i,$len);
         $arb = strpos($dirx,'@');
         $pto = strpos($dirx,'.',$arb);
         $i = $ini+strlen($dirx);
         $ini = $i;
...
//         if ($arb>0 and $pto>0) {
           ...
           $quer2 = "insert into mensajes (codigo,mensaje) values ('".$codigo."','".$dirx.' 2 '.strval($pos).' '.strval($i).' '.strval(strlen($emails))."')";
//            $quer2 = "insert into mensajes (codigo,mensaje) values ('00000','".$dirx."')";
            echo $quer2.'<br><br>';
            //$quer2 = "insert into mensajes (codigo,mensaje) values ('00000','prueba')";
//           $resul2 = mysql_query($quer2,$link);
