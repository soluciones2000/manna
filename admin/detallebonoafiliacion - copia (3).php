...
if (isset($mes) and isset($ano)) {
	...
	while($row = mysql_fetch_array($result)) {
		if ($patroc_codigo<>$row["patroc_codigo"]) {
			...
			$quer6 = "SELECT tit_nombres,tit_apellidos,tipo_afiliado from afiliados where tit_codigo='".$patroc_codigo."'";
			$resul6 = mysql_query($quer6,$link);
			if($ro6 = mysql_fetch_array($resul6)) {
				$nom = $ro6["tit_nombres"];
				$ape = $ro6["tit_apellidos"];
				$taf = $ro6["tipo_afiliado"];
				$namept = utf8_encode(trim($nom)." ".trim($ape));
			} else {
				$namept = "";
			}
		    echo "<b><u>Patrocinador: ".$patroc_codigo." ".$namept." ".$taf."</u></b><br>";
			echo "</div>";
		}
